...
export default {
  ...
  gclogFilePlaceholder:
    '填写文件分析页面的URL，如果要选择分析的时间段请在文件分析页面里先配置好。可以对比同一个GC日志的两个不同时间段',
  ...
  targetFile: '目标日志',
  metric: '指标',
  metricValueOfBaseline: '基线值',
  metricValueOfTarget: '目标值',
  metricValueDifference: '差异（%）',
  metricCompare:
    "目标 VS 基线 (<font color='#9ad969'>绿色</font>表示变好，<font color='#E74C3C'>红色</font>表示变差)",
  betterPerformance: '性能变好',
  worsePerformance: '性能变差',
  parallelGCThreads: 'GC并行线程数',
  parallelGCThreadsHint:
    'GC并行线程(对应参数-XX:ParallelGCThreads)用于处理所有暂停阶段的工作，增大线程数可以减少GC暂停时间，但不要超过CPU数量。',
  concurrentGCThreads: 'GC并发线程数',
  concurrentGCThreadsHint:
    'GC并发线程(对应参数-XX:ConcGCThreads)用于所有并发阶段的工作，增大线程数可以让并发GC更快完成，但是会提高并发GC过程中的CPU利用率',
  gcDetail: 'GC详情',
  applyTimeToConfig: '将当前所选时间段应用到整个页面',
  noDatestampPreunified:
    '日志中没有打印GC发生的现实时间，只能根据相对于日志开始的时间进行搜索，建议增加参数-XX:+PrintGCDateStamps来打印现实时间。',
  noDatestampUnified:
    '日志中没有打印GC发生的现实时间，只能根据相对于日志开始的时间进行搜索，建议把GC日志打印的参数修改为类似 -Xlog:gc*:file=gc.log:time,uptime,level,tags (加上time这一项)来打印现实时间',
  ...
  memoryStats: {
    ...
    capacityAvg: '平均容量',
    capacityAvgHint:
      '容量表示这个区域能最大容纳多少对象。平均容量有可能会比最大使用量更小，原因在于容量可以是动态变化的，比如当-Xmx!=-Xms时堆会根据内存使用情况和吞吐动态进行扩缩容，年轻代和老年代大小也会相应进行调整',
    g1DynamicCapacity:
      '此外，即使-Xmx=-Xms，对于G1而言只要没有设-Xmn(通常也不推荐设Xmn)的话年轻代和老年代大小也是动态的',
    usedMax: '最大使用量',
    usedMaxHint:
      '可以用来评估java进程整体内存占用的RSS，其中堆所占的大小比较接近堆历史上最高使用量',
    usedAvgAfterFullGC: 'Full GC后平均使用量',
    usedAvgAfterFullGCHint:
      '用于估算这个区域大概有多少常驻的对象，如果这个值较高，说明应用在该区域存在内存泄漏的可能性',
    usedAvgAfterOldGC: 'Old GC后平均使用量',
    usedAvgAfterOldGCHint:
      '用于估算这个区域大概有多少常驻的对象，如果这个值较高，说明应用在该区域存在内存泄漏的可能性',
    metaspaceCapacity:
      '注意原始GC日志中打印的类似 "Metaspace: 1792694K->291615K(698368K)" 的信息表示元空间reserve的空间是698368K,而非真正的容量',
  ...
  generation: {
    ...
    humongous: '大对象',
    humongousHint:
      'G1的大对象指的是大小超过Region大小(对应参数-XX:G1HeapRegionSize)一半的对象，如果大对象过多可能会引发GC异常，通常通过调大Region或者减少大对象创建优化',
    archive: 'CDS区',
    heap: '整堆',
    metaspace: '元空间'
  },
  ...
  cause: {
    ...
    cmsFinalRemark:
      '如果设置了-XX:+CMSScavengeBeforeRemark参数，那么会在CMS Final Remark时先做一次Young GC，通常开启此参数可以减少Remark的最大暂停时间',
    systemgc: '调用System.gc()或Runtime.getRuntime().gc()时触发',
    jvmti: '使用Jvmti时调用ForceGarbageCollection触发',
    gclocker:
      '如果线程执行在JNI临界区时，刚好需要进行GC，此时GC Locker会阻止GC的发生，同时阻止其他线程进入JNI临界区，直到最后一个线程退出临界区时触发这种GC。过于频繁使用GCLocker可能会使得gc无法及时进行，可能导致长时间暂停甚至可能导致Full GC',
    heapInspection: '当你使用工具(如jmap)执行堆检查操作时会触发',
    heapDump: '执行Heap Dump前触发的一次Full GC',
    allocationFail: '对象分配空间不足时触发，这是最常见的GC触发原因',
    metaspace:
      '元空间不足时触发。注意由于元空间存在碎片化的问题，可能看起来空间没有用满但依然无法分配',
    ergonomics: '是为了调整堆的大小，从而达到某种暂停时间或吞吐量目标',
    g1Evacuation: '对象分配空间不足时触发，这是最常见的GC触发原因',
    humongous: 'G1大对象分配失败时触发Full GC',
    lastDitch:
      '在Metadata GC Threshold GC后，如果元空间依然不足，就会再次触发Full gc并清除弱引用',
    promotionFail:
      '在Young GC过程中，如果老年代没有足够的空间提供晋升，那么这次GC就会变成Full GC',
    toSpaceExhausted:
      '在Young/Mixed GC过程中，没有足够的region作为to space或用来晋升的现象称为To-space exhausted，这可能会导致空间无法正常释放, 大量对象晋升以及Young GC的暂停时间大幅度拉长',
    proactive: 'JVM主动触发GC来减少堆内存占用，通常只在对象分配速率很低时进行',
    allocationRate: '当JVM根据当前的对象分配速度估计内存即将耗尽，就会触发一次GC',
    timer:
      '如果设置了-XX:ZCollectionInterval，如果超过了该参数的时长都没有触发GC则主动触发一次GC',
    allocationStall: '对象分配空间不足时触发',
    highUsage: '堆的使用率超过一定的百分比，就会触发一次GC',
    warmup:
      'JVM启动之后，如果一直没有发生过GC，那么会在堆内存使用超过10%、20%、30%时，分别触发一次GC',
    metaspaceClearSoftRef:
      '在Metadata GC Threshold GC后，如果元空间依然不足，就会再次触发Full gc并清除弱引用',
    g1Periodic:
      '如果设置了-XX:G1PeriodicGCInterval，如果超过了该参数的时长都没有触发GC则主动触发一次GC，通常这个特性用于进行堆内存的回收',
  ...
  phase: {
    ...
    concMarkAbort:
      'Concurrent Mark Abort表示在Concurrent Cycle过程中发生了Full GC，本次Concurrent Cycle中止，通常需关注Full GC的发生原因',
    concModeFailure:
      'Concurrent Mode Failure表示在CMS GC过程中发生了Full GC，本次CMS GC中止，通常需关注Full GC的发生原因',
    concModeInterrupt:
      'Concurrent Mode Interrupted表示在CMS GC过程中发生了System.gc()导致的Full GC，本次CMS GC中止，通常需关注Full GC的发生原因',
    allocationStall: 'Allocation Stall表示当前线程无法继续分配对象，被迫等待直至GC完成',
    cmReset:
      'Concurrent Mark Reset For Overflow表示全局标记栈已满，将重新开始标记，可能导致Concurrent Cycle时间超长。如果出现了这种情况则建议调大-XX:MarkStackSize参数(默认值4M)',
  ...
  badHint: {
    ...
    badPause: '当前暂停时间偏长',
    badUsageAfterGC: '在GC后使用率仍然较高，可能容量设置得过低或者存在内存泄漏的情况',
  ...
  diagnose: {
    ...
    suggestion: {
      upgradeTo11G1FullGC:
        '如果实在没办法消除Full GC或者认为Full GC是可以接受的，可以考虑到升级到JDK11的G1 GC，JDK11中G1的Full GC改成了多线程进行，能有效减少暂停时间',
      checkSystemGC: '检查一下System.gc()是在哪里调用的，是否有必要',
      disableSystemGC: '可以增加参数 -XX:+DisableExplicitGC 来彻底关闭System.gc()',
      oldSystemGC:
        '或者考虑增加参数 -XX:+ExplicitGCInvokesConcurrent 使得调用System.gc()触发一次并发的Old GC',
      ...
      increaseZAllocationSpikeTolerance:
        '适当增大参数 -XX:ZAllocationSpikeTolerance，使得GC更早发生',
      checkLiveObjects:
        '如果Young GC时的活对象多导致活对象拷贝时间长或者引发跨代引用，那么考虑排查一下应用逻辑，为什么会有那么多活对象',
      checkReferenceGC:
        '如果是Reference Processing时间长，考虑开启-XX:+ParallelRefProcEnabled多线程优化(JDK11后默认开启)，或者排查为何有这么多Weak/Soft/Phantom Reference',
      checkCPUTime:
        '检查GC的CPU时间，如果real时间长而usr，sys时间短，可能是磁盘IO或者同一宿主机上不同进程或容器互相干扰导致的',
      shrinkYoungGen: '适当减小参数-Xmn，来减少年轻代大小',
      shrinkYoungGenG1: '适当减小参数-XX:G1MaxNewSizePercent，来减少年轻代大小',
      checkEvacuationFailure:
        '如果GC出现了To-space exhausted的情况，会大幅拉长GC暂停，解决方法一方面是G1尽量不要设Xmn参数，另一方面考虑排查是否存在大对象分配过多或者内存泄漏或者晋升过快等情况',
      checkMemoryLeak: '通过Heap Dump等方式排查是否存在内存泄漏的情况',
      checkFastPromotion:
        '如果这次Full GC前老年代增长地较快，考虑通过做Full GC前的Heap Dump来分析是否存在长生命周期对象引起的过早晋升',
      checkRescan: '如果是Rescan的时间长，可以添加-XX:+CMSScavengeBeforeRemark来优化',
      checkClassUnloading:
        '如果是Class Unloading的时间长，有可能需要优化下代码，减小class loader的数量',
      expandYoungGen: '适当增加参数-Xmn，来增大年轻代大小',
      expandYoungGenG1:
        '考虑增大年轻代大小。G1的年轻代大小比较复杂，是根据应用暂停时间来动态调整的，以尽量满足暂停目标。如果当前暂停时间较长，则先考虑优化暂停时间，或者提高暂停时间目标(即增大参数-XX:MaxGCPauseMillis,默认值200 ms)。也可以考虑提高年轻代大小的下限（即打开-XX:+UnlockExperimentalVMOptions，提高-XX:G1NewSizePercent,默认值5，表示年轻代最小是整堆的5%）',
      checkFastObjectAllocation: '排查一下代码中是否存在对象分配速度过快的情况，优化代码',
      useMoreDetailedLoggingUnified:
        '如果当前的信息还没法确定具体的原因，可以考虑打开debug级GC日志以获取更多信息(把-Xlog参数中的gc*改成gc*=debug，debug级日志内容过多，不建议长期开启)'
    }
  },
